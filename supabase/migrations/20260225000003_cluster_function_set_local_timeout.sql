-- Set statement_timeout = 0 inside the function so the expensive pairwise
-- cosine comparison is not killed by PostgREST's per-request default limit.
...
as $$
begin
  -- Disable statement timeout for this call so the pairwise cosine query
  -- can run to completion (can take 10–40s for large windows).
