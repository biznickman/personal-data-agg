...
- `assign.ts` (`x-news-cluster-assign`, event: `x-news/tweet.normalized`)
  ...
  - In embedding mode, lazily writes `tweets.normalized_headline_embedding` via Gemini
  - Builds canonical text + token set
  - Attaches tweet to best recent cluster (embedding cosine or Jaccard) or creates a new cluster
...
- `merge.ts` (`x-news-cluster-merge`, cron: every 2 minutes)
  ...
  - Merges highly similar clusters (embedding cosine or Jaccard fallback)
...
- `GEMINI_API_KEY` (required for embedding mode)
